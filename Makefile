...
seed-users-and-accounts: ## Seed Users and User accounts directly to the database
	go run $(USER_SEED_FILE) -noOfUsers 100000

.PHONY: seed-orders
seed-orders: ## Seed orders via order-api
	go run $(ORDER_SEED_FILE) -noOfOrders 100000 -noOfOrdersPerAccount 1 -rps=500 -orderApiUrl http://order-api.localtest.me # http://order-api.localtest.me, http://localhost:8081
