...
func TraceID(logger *zap.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if utils.IsEmpty(requestIDStr) {
			logger.Error("request_id_is_missing_in header", zap.String("method", c.Request.Method), zap.String("path", c.FullPath()))
		...
		if err != nil {
			logger.Error("failed_to_parse_request id", zap.String("method", c.Request.Method), zap.String("path", c.FullPath()))
